...
const PlanningDispatch = () => {
  const { Drows, setDrows, Masterdata, busFilteredRows } = useContext(AuthContext)
  ...
  const handleAddRow = () => {
    ...
    const isItemExist = Drows.some(item => item.FertCode === selectedFertCode.item_code)
    ...
    if (selectedFertCode.item_code === undefined) {
      ...
      setDrows(prevRows => [...prevRows, newRow])
  ...
  //     Quantity: quantity
  //   }
  //   setDrows(prevDrows => [...prevDrows, newRow])
  //   setSelectedFertCode(null)
  //   setQuantity(0)
  // }
  ...
  const DispatchSubmit = () => {
    ...
    try {
      ...
      const response = fetch('/api/Diversitech/Supervisors/Dispatch/Createplanning', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ Drows })
      })
        ...
        .then(data => {
          ...
          setTimeout(() => {
            window.location.href = '/components/Diversitech/Supervisors/PlanView/Dispatch/' // This will reload the page
          }, 1000)
        })
        .catch(error => {
          console.error(error)
  ...
  const handleDeleteRow = index => {
    setDrows(prevDrows => prevDrows.filter((row, i) => i !== index))
  }

  const [selectedFertCode, setSelectedFertCode] = useState(null)

  // console.log('====' + selectedFertCode)
  // console.log(selectedFertCode)
    ...
    <>
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        ...
        <DialogContent>
          ...
          <Grid container justifyContent='center' alignItems='center'>
            <Grid padding='20px'>
              <Button size='medium' onClick={DispatchSubmit} variant='contained'>
                Yes
              </Button>
            </Grid>
            <Grid padding='10px'>
...
export default PlanningDispatch
