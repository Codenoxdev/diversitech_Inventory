...
const AnalyticsTotalTransactions = ({ data }) => {
    ...
    <Card>
      <Grid container>
        <StyledGrid item xs={12} sm={7}>
          <CardHeader title='Weekly charts' titleTypographyProps={{ sx: { letterSpacing: '0.15px' } }} />
          <CardContent
            sx={{
          ...
          >
            {data && Array.isArray(data) ? (
              <ReactApexcharts type='bar' height={278} series={data} options={options} />
            ) : null}
          </CardContent>
        </StyledGrid>
        <Grid item xs={12} sm={5}>
          <CardHeader
            title='Report'
            subheader='Last month seats dispatched 1000'
