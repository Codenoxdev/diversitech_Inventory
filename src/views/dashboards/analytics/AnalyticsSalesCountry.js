...
const AnalyticsSalesCountry = ({ topFive }) => {
  console.log(topFive)

  // ** Hook
  const theme = useTheme()

  const itemCodes = topFive ? topFive.map(transaction => transaction.item_code) : null
  const data = topFive ? topFive.map(transaction => transaction.total_qty) : null
  console.log(data)
  ...
  const options = {
    ...
    xaxis: {
      ...
      categories: itemCodes, //['IU363291', 'IU457236', 'IU363286', 'IU457235', 'IU463580'],
      labels: {
  ...
  return (
    <Card>
      <CardHeader
        title='Top Seats Productions'
        // subheader='Total 210 Seats Dispatched'
