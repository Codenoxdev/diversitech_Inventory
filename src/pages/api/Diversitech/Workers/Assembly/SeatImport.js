...
export default async function handle(req, res) {
  try {
    const { item_code, count_value, plan_no, type, frame_id } = req.body
    ...
    SET seat_prod_qty = LEAST(COALESCE(seat_prod_qty, 0) + ?, frame_paint_qty),updated_on = ?
    ...
    connection.query(SelectQuery, [item_code], async (err, itemResult) => {
      ...
      connection.query(SelectCheckQuery, [item_id, plan_no], async (err, checkResult) => {
        ...
        for (const frame of checkResult) {
          const { frame_paint_qty, seat_prod_qty } = frame

          // console.log(frame_paint_qty, seat_prod_qty)
          // console.log(parseInt(frame_paint_qty) + parseInt(count_value))

          if (
            // frame_plan_qty === parseInt(frame_prod_qty) + count_value &&
            frame_paint_qty <
            parseInt(seat_prod_qty) + parseInt(count_value)
          ) {
            canInsert = false
            break
          }
